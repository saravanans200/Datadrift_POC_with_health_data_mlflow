...
input_data = pd.read_csv(csv_path('//data//healthcare_dataset_updated.csv'))
print(input_data)
...
input_data = input_data.assign(temperature=pd.cut(input_data['Body_temperature'], bins=[90, 94, 98, 110], right=False, labels=["low", "moderate", "hign"]))
input_data.drop(columns="Body_temperature",inplace=True)
input_data.info()
...
sns.countplot(x = input_data['Test Results'], hue = input_data['temperature'])
plt.show()
...
print("***********************************")
encoded_data[['Age_group', 'bill_group', 'temperature']] = encoded_data[['Age_group', 'bill_group', 'temperature']].astype('int64')
encoded_data.to_csv(csv_path('//data//meta_data(data_cleaned)_after_drift_detection.csv'),index=False)
